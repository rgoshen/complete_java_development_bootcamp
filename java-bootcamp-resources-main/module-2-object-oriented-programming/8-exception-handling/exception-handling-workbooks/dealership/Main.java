import java.util.Scanner;

import models.Car;
import models.Dealership;

public class Main {
    public static void main(String[] args) {

        Scanner scan = new Scanner(System.in);

        Car[] cars = new Car[] {
            new Car("Nissan", 5000),
            new Car("Cadillac", 12000),
        };
        cars[1].setMake("Dodge");
        cars[1].setPrice(8500);

        Dealership dealership = new Dealership(cars);

        System.out.println("\n************* JAVA DEALERSHIP *************");
        while (true) {
            if (dealership.isEmpty()) {
                System.out.println("\nSorry, we are all sold out.");
                break;
            }

            System.out.println(dealership);
            System.out.print("Enter the spot number of the car you want to buy: ");
            if (!scan.hasNextInt()) {
                scan.nextLine();
                System.err.println("INVALID INPUT");
                continue;
            }
            int spot = scan.nextInt();
            scan.nextLine();
            if (spot < 0 || spot > dealership.getLength() - 1) {
                System.err.println("Please choose a valid parking spot.");
                continue;
            } else if (dealership.getCar(spot) == null) {
                System.err.println("Spot " + spot + " is empty.");
                continue;
            }
            dealership.sell(spot);

            System.out.print("Type 'yes' to continue shopping: ");
            if (!scan.nextLine().toLowerCase().equals("yes")) {
                break;
            }
        }
        
        System.out.println("\nThank you for shopping at Java Dealership. Please come again.");

    //    scan.close();

    }
}
